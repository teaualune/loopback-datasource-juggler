...
DataAccessObject.findById = function findById(id, filter, options, cb) {
  ...
  if (options === undefined && cb === undefined) {
    ...
    if (typeof options === 'function') {
      ...
      cb = options;
      options = {};
      if (typeof filter === 'object' && !(filter.include || filter.fields || filter.where)) {
        // If filter doesn't have include or fields or where, assuming it's options
        options = filter;
        filter = {};
      }
    }
  ...
  if (isPKMissing(this, cb)) {
    ...
    if (filter.where) {
      util._extend(query.where, filter.where);
    }
